...
async def reply(context: commands.Context, content: str, Ephermal=False):
    ...
    if permissions.send_messages:
        ...
        if canDm:
            await context.author.send(
                "ei, eu não consigo a menssagem no canal a onde tu")
...
def getEnergy(guild_id: int):
    with sql.connect("settings.db") as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT energy FROM server_settings WHERE guild_id = ?",
                       (guild_id, ))
        ...
        if result is not None:
            ...
            cursor.execute(
                "INSERT INTO server_settings (guild_id, language, energy) VALUES (?, ?, ?)",
                (guild_id, 'Portuguese', 100))
            conn.commit()
            return 100
...
def setEnergy(guild_id: int, energy: int):
    with sql.connect("settings.db") as conn:
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE server_settings SET energy = ? WHERE guild_id = ?",
            (energy, guild_id))
        conn.commit()
...
def substractEnergy(guild_id: int, amount: int):
    with sql.connect("settings.db") as conn:
        ...
        if result is not None:
            ...
            new_energy = max(0, current_energy -
                             amount)  # evita que fique negativo
            cursor.execute(
                "UPDATE server_settings SET energy = ? WHERE guild_id = ?",
                (new_energy, guild_id))
            conn.commit()
...
async def sleep(context: commands.Context, time: int):
    ...
    sleeping = False
    await sendMessage(
        f"uah... bom dia pessoal, como vão? tambem, {context.author} obrigado por fazer eu dormir",
        context.channel)
...
class commandView(discord.ui.View):
    ...
    @discord.ui.button(label="Saber mais do sistema de energia",
                       style=discord.ButtonStyle.primary,
                       emoji="💡")
    async def button(self, interaction: discord.Interaction,
                     button: discord.Button):
        await interaction.response.send_message(
            "eu tenho um sistema de energia que certos comandos fazem eu perder certa energia, tipo o !ping que eu perco 1 de energia, alguns não gastam a energia tipo o comando de saber a minha eneriga; para eu ganhar energia você pode fazer o comando !sleep que eu durmo por o restante da energia ate chegar a 100% (exemplo eu tehno 38% de energia e você fala o comando !sleep, eu vou demorar 62 segundos (100 - 38) para acordar e começar a funcionar)",
            ephemeral=True)
...
class youtubeView(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction,
        ...
        if self.user != interaction.user:
            await interaction.response.send_message(
                "você não é o usuário que eu esperava, sai daqui!",
                ephemeral=True)
            return

        if is_connected(self.bot, interaction.guild) is not None:
            await interaction.response.send_message(
                "desculpa, mas já estou em uma call, quando eu acabar, por favor me chame novamente!",
                ephemeral=True)
            self.cancelled = True
            self.stop()
            return
        ...
        await interaction.response.send_message(
            "certo, entrando no voice chat que tu está")
        self.ended = True
        self.stop()
...
def run():
    ...
    async def on_ready():
        await bot.change_presence(
            ...
            activity=discord.Game("minhas speedruns do minecraft"))
        createServerTable()
    ...
    async def on_guild_join(guild):
        for channel in guild.text_channels:
            if channel.permissions_for(guild.me).send_messages:
                await channel.send(
                    'ola, sou o steve do minecraft e estou aqui para fazer coisas, se quiser saber os meus comandos so falar !commands'
                )
                break
    ...
    async def comandos(context: commands.Context):
        ...
        embed.add_field(
            name='!ping',
            value=
            'eu falo o meu ping para ver o quao lento estou e se eu estou acordado'
        )
        embed.add_field(
            name='!energy',
            value=
            'eu tenho um sistema de energia que é so clicar no botão de "Saber mais do sistema de energia" para aprender mais, eu so falo o quao de energia que eu tenho'
        )
        embed.add_field(
            name='!sleep',
            value=
            'a explicação deste comando esta no "Saber mais do sistema de energia"'
        )
        embed.add_field(
            name='!youtube (link de um video do youtube)',
            value=
            "se estiver numa voice chat, eu irei entrar e tocar o link do youtube, como audio obvio, o audio é distorçido"
        )
        embed.add_field(
            name="!cantar",
            value=
            "se estiver numa voice chat eu irei cantar uma musica que o meu criador me ensinou"
        )
        embed.add_field(
            name="!stevobanir (uma menção o usuario escolhido para matar)",
            value=
            "(SOMENTE PARA CARGOS COM ADMINISTRADOR) eu bano o usuario selecionado com um aviso"
        )
    ...
    async def ping(context: commands.Context):
        ...
        if energyVariable >= energyAmmount:
            ...
            await reply(
                context,
                "não tenho energia para pode te dar ping... quero dormir...",
                Ephermal=True)
            return
        await reply(
            context,
            "bom dia vossa excelencia, demorei {}ms para responder".format(
                bot.latency),
            Ephermal=True)
    ...
    async def youtube(context: commands.Context, url=None):
        ...
        try:
            ...
            await context.send(
                "mas você não está em um voice chat... como que eu vou cantar a música?"
            )
            return
        ...
        if energyVariable >= energyAmmount:
            ...
            await context.send(
                "não tenho energia para tocar os video do youtube... quero dormir..."
            )
            return
    ...
    async def dormir(context: commands.Context):
        ...
        if energia:
            if energia < 100:
                await reply(
                    context,
                    f"uah... vou dormir por {100-energia} segundos, ate..")
                await sleep(context, 100 - energia)
            else:
                await reply(
                    context,
                    "mas ja estou com full energia, não preciso dormir",
                    Ephermal=True)
    ...
    async def cantar(context: commands.Context):
        ...
        if energyVariable >= energyAmmount:
            ...
            await context.send(
                "não tenho energia para poder cantar... quero dormir...")
            return
            ...
            if usuario.split("<@")[1] and usuario.split("<@")[1].split(">")[0]:
                ...
                if context.author.guild.get_member(int(userId)):
                    ...
                    if banUsuario.guild_permissions.ban_members or banUsuario.guild_permissions.administrator:
                        await context.send("não posso banir adms",
                                           ephemeral=True)
                        return
                    try:
                        await banUsuario.ban(
                            reason=
                            f"steve matou você porcausa do {context.author}")
                    except Exception as e:
                        await context.send(
                            f"erro, não consegui banir o usuario porcausa do seguinte erro: {e}"
                        )
                        return
                    try:
                        dmChannel = await banUsuario.create_dm()
                        await dmChannel.send(
                            f"eu te matei (bani) do servido {context.author.guild.name} com as ordens do usuario {context.author.name}"
                        )
                    except Exception as e:
                        print(e)
            else:
                await context.send(
                    "você fez alguma coisa errada na menção do usuario, por favor corrija e mande o comando novamente",
                    ephemeral=True)
                return
        else:
            await context.send(
                "a onde esta o usuario? tem nada, mande com o usuario desta vez",
                ephemeral=True)
            return
    ...
    async def energy(context: commands.Context):
        ...
        if energyToSpeak >= 50:
            ...
            await context.send(
                f"estou com {str(energyToSpeak)}% de energia, estou meio cansado pode me botar para dormir?"
            )
        else:
            await context.send(
                f"*bocejo* estou com {str(energyToSpeak)}% de energia, quero dormir porfavor.."
            )
